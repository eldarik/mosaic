...
import { ConfidentialKeys } from '@solana/mosaic-sdk/_zk';
...
// `ConfidentialKeys.signerMessage(seed)` is always `domainSeparator + seed`;
// calling it with an empty seed yields the bare domain separator, which every
// canonical key-derivation message must start with regardless of what seed
// (token account, owner, owner+mint, ...) is in use. This lets
// `assertCanonicalDerivationMessage` recognise a real derivation message
// without hard-coding — or needing to agree with upstream on — the seed
// scheme itself.
const MESSAGE_PREFIX = ConfidentialKeys.signerMessage(new Uint8Array(0));
...
function assertCanonicalDerivationMessage(message: Uint8Array): void {
    if (hasPrefix(message, MESSAGE_PREFIX)) return;
    throw new Error(
