...
describe('deriveConfidentialKeys', () => {
    ...
    it('matches the cross-SDK standard vector', async () => {
        ...
        const keys = await deriveConfidentialKeys({ signer });
        expect(new Uint8Array(keys.elgamal.secret().toBytes())).toEqual(
            new Uint8Array([
                0xbe, 0x5c, 0xce, 0x95, 0x1f, 0x42, 0xa2, 0xa8, 0x67, 0x7d, 0x1a, 0x56, 0xf0, 0x3a, 0xae, 0x7b, 0xff,
                0x79, 0x5b, 0x38, 0xcf, 0x1c, 0x56, 0xc8, 0xcf, 0x3a, 0x4d, 0xae, 0x7d, 0x60, 0xe2, 0x05,
            ]),
        );
        expect(new Uint8Array(keys.aes.toBytes())).toEqual(
            new Uint8Array([0x64, 0x17, 0xee, 0xdb, 0xcb, 0xe9, 0xc6, 0x4a, 0x72, 0x39, 0x57, 0x19, 0xec, 0x98, 0xcf, 0x6b]),
        );
        freeConfidentialKeys(keys);
    });
