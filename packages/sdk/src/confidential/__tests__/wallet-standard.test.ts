...
// domain separator).
const MESSAGE = ConfidentialKeys.signerMessage(new Uint8Array([1, 2, 3]));
...
describe('signMessageViaWalletStandard: canonical message enforcement', () => {
    it('accepts a canonical derivation message with a different seed', async () => {
        const signature = fakeSignature(9);
        const otherSeedMessage = ConfidentialKeys.signerMessage(new Uint8Array([4, 5, 6]));
        setWallets([
            fakeWallet(
                OWNER,
        ...
        const result = await signMessageViaWalletStandard(OWNER, otherSeedMessage);
