...
describeSkipIf(!RUN)('confidential transfer (devnet e2e)', () => {
    ...
    it('runs configure → deposit → apply → transfer → withdraw → empty with decrypted balance checks', async () => {
        ...
        // 1. Create the mint with confidential balances (opt-in / auto-approve so
        //    both accounts are usable immediately, no manual approval step).
        const createMintTx = await new Token()
            .withConfidentialBalances({ authority: payer.address, policy: 'auto-approve' })
            .buildTransaction({
